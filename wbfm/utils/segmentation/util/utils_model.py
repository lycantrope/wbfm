...
def segment_with_stardist_2d(
    vol: np.ndarray, model=None, zero_out_borders=False, verbose=0
) -> np.ndarray:
    """
    ...
    for idx, plane in enumerate(vol):
        ...
        if zero_out_borders:
            ...
            labels_bd = find_boundaries(
                labels, connectivity=2, mode="outer", background=0
            )
...
def segment_with_stardist_3d(
    volume: np.ndarray, model: stardist.models.StarDist3D, verbose: object = 0
) -> np.ndarray:
    """
